...
class ParawingsTable {
      ...
      this.parawings = data.flatMap((parawing, parawingIndex) =>
        parawing.sizes.map((size, sizeIndex) => {
          ...
          if (parawing.reviews && parawing.reviews.length > 0) {
            ...
            const newReviewsStars = parawing.reviews.reduce((sum, review) => sum + review.stars, 0)
            
            // Update the total count
            const newTotalCount = reviewsCount + parawing.reviews.length
            
            // Calculate the new average rating
            averageRating = (totalExistingStars + newReviewsStars) / newTotalCount
  ...
  updateConditionalFilters() {
    ...
    const hasRatings = this.parawings.some(item => item.reviewsCount > 0 && item.averageRating > 0)
  ...
  applyFiltersAndSort() {
    ...
    if (this.minRating !== "") {
      result = result.filter((item) => item.reviewsCount > 0 && item.averageRating >= Number(this.minRating))
    }

    if (this.maxRating !== "") {
      result = result.filter((item) => item.reviewsCount > 0 && item.averageRating <= Number(this.maxRating))
    }
    
    // Apply reviews count range filter
    if (this.minReviews !== "") {
      result = result.filter((item) => item.reviewsCount >= Number(this.minReviews))
    }

    if (this.maxReviews !== "") {
      result = result.filter((item) => item.reviewsCount <= Number(this.maxReviews))
    }
        ...
        Object.keys(this.visibleColumns).forEach((column) => {
          if (this.visibleColumns[column]) {
            ...
            if (column === "name") {
              ...
              cell.textContent = parawing.reviewsCount > 0 ? parawing.reviewsCount : "-"
            } else {
